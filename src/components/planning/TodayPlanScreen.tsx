...
export async function TodayPlanScreen({
  requestedStep, embedded = false, returnHref = "/dashboard",
}: {
  requestedStep?: string; embedded?: boolean; returnHref?: string;
}) {
  ...
  const { data: profileData, error: profileError } = await supabase
    ...
    .eq("id", user.id)
    .maybeSingle();
  if (profileError) throw new Error('Your planner settings could not be loaded.');
  ...
  // A failed read must not become an empty plan that can overwrite saved work.
  if ([tasksResult, habitsResult, habitLogsResult, templatesResult, entriesResult, planResult].some((result) => result.error)) {
    throw new Error('Your daily plan could not be loaded. Please try again.');
  }
