...
- **Dashboard hero illustrated under Trail** (branch `feat/trail-dashboard-illustration`, off `feat/trail-theme`) — `TrailDashboardHero` renders the same real level/XP/coins/streak data as the existing hero inside a hand-drawn scene (two overlapping mountains, three pine trees, a dashed path with a waypoint marker) with a Fraunces display heading scoped to just this component. Labels were kept as-is (Level, XP, coins, streak), not renamed to the mockup's vocabulary. A new `ThemedDashboardHero` switches between it and the existing `DashboardHero` (left untouched) based on the active theme, so White/System/Dark carry zero risk. Verified in the browser via the real Settings UI, not a DOM-class shortcut.

None of the four branches (`feat/trail-theme`, `feat/hide-city`, `feat/avatar-profile-merge`, `feat/trail-dashboard-illustration`) are merged into `master` yet.
